...
int main(){
    ...
    while(1){
        ...
        for(int i = 0; i < contours.size(); ++i){
            if(contourArea(contours[i]) > 1000){//moving object detected
                count++;
                moving_detect = 3;
            }
        }
        ...
        if(moving_detect > 0){
            putText(live,format("Alert moving object detected"),Point(30,30),FONT_HERSHEY_SIMPLEX,1,Scalar(0,0,255),4);
        }
